...
function parseFromHTML(html) {
  ...
  let i = 0, date = '', title = '', quote = '', source = '', body = [], aff = '';

  // Ищем дату
  while (i < lines.length && !dateRe.test(lines[i])) i++;
  if (i < lines.length) date = lines[i++];

  // Заголовок — следующая строка после даты (короткая, не цитата)
  while (i < lines.length && lines[i].length < 3) i++;
  if (i < lines.length && lines[i].length < 80 && !lines[i].startsWith('«') && !sourceRe.test(lines[i])) {
    title = lines[i++];
  }

  // Цитата — строка в кавычках
  while (i < lines.length && lines[i].length < 3) i++;
  if (i < lines.length) {
    const l = lines[i];
