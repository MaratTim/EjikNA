...
function parseFromHTML(html) {
  // Ищем заголовок через H1/H2/H3 теги до очистки HTML
  let title = '';
  const titleMatch = html.match(/<h[123][^>]*>\s*([^<]{3,80})\s*<\/h[123]>/i);
  if (titleMatch) {
  ...
  // Если заголовок не нашли через H тег — ищем как короткую строку после даты
  if (!title) {
    ...
    // Пропускаем строку если она совпадает с найденным заголовком
    while (i < lines.length && lines[i].length < 3) i++;
